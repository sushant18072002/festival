...
import { HomeGreeting } from '../../lib/models';
...
export default async function handler(
    ...
    if (req.method === 'GET') {
        try {
            ...
            const total = await HomeGreeting.countDocuments(filter);
            const greetings = await HomeGreeting.find(filter)
                .sort({ updatedAt: -1 });
        ...
        try {
            const greeting = await HomeGreeting.create(req.body);
        ...
        try {
            ...
            const greeting = await HomeGreeting.findByIdAndUpdate(_id, updateData, { new: true });
        ...
        try {
            ...
            if (permanent === 'true') {
                await HomeGreeting.findByIdAndDelete(id);
                res.status(200).json({ success: true, message: 'Permanently deleted' });
            } else {
                await HomeGreeting.findByIdAndUpdate(id, { is_deleted: true, deleted_at: new Date() });
