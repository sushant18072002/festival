        ...
        <AnimatePresence>
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                <motion.div
                    ...
                    className="bg-slate-900 rounded-2xl shadow-2xl w-full max-w-[95vw] lg:max-w-7xl h-[90vh] overflow-hidden flex flex-col"
                >
                    <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900 shrink-0 z-10">
                        <div>
                    ...
                    <div className="flex flex-1 overflow-hidden min-h-0">
                        ...
                        <div className="flex-1 flex flex-col border-r border-slate-800 min-w-0 bg-slate-900">
                            {/* Tabs Header */}
                                        ...
                                        <div className={clsx("space-y-8", activeLang !== 'en' && "opacity-50 pointer-events-none grayscale")}>
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-4">
                                                    ...
                                                    {/* MULTIPLE DATES CONFIG */}
                                                    <div className="pt-2 border-t border-slate-800">
                                                        <div className="flex justify-between items-center mb-2">
                                                            <label className="block text-xs font-bold text-slate-300">Anniversary / Recurring Dates</label>
                                                            <button type="button" onClick={addDate} className="text-xs text-blue-500 hover:text-blue-400 font-medium">+ Add Date</button>
                                                        </div>
                                                            ...
                                                            {formData.dates?.map((d: any, i: number) => (
                                                                <div key={i} className="flex gap-2">
                                                                    <input
                                                                        type="number"
                                                                        placeholder="Year"
                                                                        className="w-20 border border-slate-800 p-2 rounded-lg bg-slate-900 text-xs text-white focus:outline-none focus:border-blue-500 text-center"
                                                                        value={d.year || ''}
                                                                        onChange={(e) => updateDate(i, 'year', parseInt(e.target.value) || 0)}
                                                                    />
                                                                    <input
                                                                        type="text"
                                                                        placeholder="YYYY-MM-DD"
                                                                        className="flex-1 border border-slate-800 p-2 rounded-lg bg-slate-900 text-xs text-white focus:outline-none focus:border-blue-500 font-mono"
                                                                        value={d.date ? d.date.split('T')[0] : ''}
                                                                        onChange={(e) => updateDate(i, 'date', e.target.value)}
                                                                    />
                                                                    <button type="button" onClick={() => removeDate(i)} className="text-red-500 p-2 hover:bg-red-500/10 rounded-lg">
                                                                        <X className="w-4 h-4" />
                                                                    </button>
                                                                </div>
                                                            ))}
                                                    ...
                                                    <div className="pt-2 border-t border-slate-800">
                                                        ...
                                                        >
                                                            ...
                                                            {audios && audios.map((a: any) => (
                                                                <option key={a._id} value={a._id}>{a.title} ({a.category})</option>
                                                            ))}
                                                        </select>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="col-span-full border border-slate-800 rounded-2xl p-6 bg-slate-950">
                                                    <div className="flex justify-between items-center mb-4">
                                                        <div>
                                                            ...
                                                            <p className="text-xs text-slate-400">4-stage engagement triggers for the app scheduler.</p>
                                                        </div>
                                                    </div>

                                                    <div className="space-y-4">
                                                        <div>
                                                            <label className="block text-xs font-medium text-slate-400 mb-1">Discovery (30 days before)</label>
                                                            <input
                                                                type="text"
                                                                className="w-full border border-slate-800 bg-slate-900 p-2.5 rounded-lg text-sm text-white focus:outline-none focus:border-blue-500"
                                                                value={formData.notification_templates?.discovery || ''}
                                                                onChange={(e) => setFormData({
                                                                    ...formData,
                                                                    notification_templates: { ...(formData.notification_templates || {}), discovery: e.target.value }
                                                                })}
                                                                placeholder="e.g. Did you know? {fact} Tap to discover more."
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="block text-xs font-medium text-slate-400 mb-1">Countdown (7 days before)</label>
                                                            <input
                                                                type="text"
                                                                className="w-full border border-slate-800 bg-slate-900 p-2.5 rounded-lg text-sm text-white focus:outline-none focus:border-amber-500"
                                                                value={formData.notification_templates?.countdown || ''}
                                                                onChange={(e) => setFormData({
                                                                    ...formData,
                                                                    notification_templates: { ...(formData.notification_templates || {}), countdown: e.target.value }
                                                                })}
                                                                placeholder="e.g. One week left to prepare for {event}!"
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="block text-xs font-medium text-slate-400 mb-1">Eve (1 day before)</label>
                                                            <input
                                                                type="text"
                                                                className="w-full border border-slate-800 bg-slate-900 p-2.5 rounded-lg text-sm text-white focus:outline-none focus:border-emerald-500"
                                                                value={formData.notification_templates?.eve || ''}
                                                                onChange={(e) => setFormData({
                                                                    ...formData,
                                                                    notification_templates: { ...(formData.notification_templates || {}), eve: e.target.value }
                                                                })}
                                                                placeholder="e.g. Tomorrow is {event}!"
                                                            />
                                                        </div>
                                                        <div>
                                                            <label className="block text-xs font-medium text-slate-400 mb-1">Day Of (On the day)</label>
                                                            <input
                                                                type="text"
                                                                className="w-full border border-slate-800 bg-slate-900 p-2.5 rounded-lg text-sm text-white focus:outline-none focus:border-purple-500"
                                                                value={formData.notification_templates?.day_of || ''}
                                                                onChange={(e) => setFormData({
                                                                    ...formData,
                                                                    notification_templates: { ...(formData.notification_templates || {}), day_of: e.target.value }
                                                                })}
                                                                placeholder="e.g. Happy {event}! Celebrate with us."
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
                        ...
                        <div className="w-80 md:w-[400px] xl:w-[500px] flex flex-col shrink-0 bg-slate-950">
                            <div className="p-4 border-b border-slate-800 shrink-0 bg-slate-900 flex justify-between items-center">
                                ...
                                >
                                    <Plus className="w-3.5 h-3.5" /> Link Media
                                </button>
                            </div>
                            ...
                            <div className="overflow-y-auto p-4 flex-1 custom-scrollbar space-y-4">
                                {!editingId && (
                                    <div className="text-center py-6 bg-slate-900 border border-slate-800 rounded-xl">
                                        <p className="text-slate-500 text-sm">Save event first to link media.</p>
                                    </div>
                                )}
                                {editingId && relatedImages.length === 0 && (
                                    <div className="text-center py-6 bg-slate-900 border border-slate-800 rounded-xl">
                                        <p className="text-slate-500 text-sm">No images linked yet.</p>
                                    </div>
                                )}
                                {editingId && relatedImages.length > 0 && (
                                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                                        {relatedImages.map(img => (
                                            <div
                                                key={img._id}
                                                className="relative aspect-square rounded-xl overflow-hidden border border-slate-700 group hover:border-blue-500 transition-all cursor-pointer bg-slate-900"
                                            ...
                                            >
                                                ...
                                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/95 via-black/60 to-transparent p-2 pt-6">
                                                    <p className="text-white text-[10px] font-semibold truncate leading-tight drop-shadow-md">
                                                ...
                                                <div className="absolute top-1.5 right-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button
                                                        ...
                                                        className="bg-red-500/90 text-white p-1 rounded-full hover:bg-red-600 transition-colors shadow-md"
                                                        title="Unlink Image"
                                                    >
                                                        <X className="w-3 h-3" />
                                                    </button>
                    ...
                    {/* Sticky Footer */}
                    <div className="p-5 md:p-6 border-t border-slate-800 bg-slate-950 flex justify-end gap-3 shrink-0 z-20">
                        <button
                            type="button"
