...
function getTranslations(en) {
    const map = {
        ...
        "Joyful": { hi: "हर्षित", mr: "आनंदी", gu: "આનંદી", bn: "আনন্দময়", ta: "மகிழ்ச்சியான", te: "సంతోషకరమైన", kn: "ಸಂತೋಷದಾಯಕ", ml: "സന്തೋಷകരമായ" },
        ...
        "Solemn": { hi: "गंभीर", mr: "गंभीर", gu: "ગંભીર", bn: "গ গম্ভীর", ta: "கம்பீரமான", te: "గంభీరమైన", kn: "ಗಂಭೀರ", ml: "ഗൗരവമുള്ള" },
...
for (let i = 50; i < 200; i++) {
    const line = lines[i];
    if (!line) continue;
    // We look for: translations: { en: { name: 'EnglishWord' }, hi: { name: '...' } }
    // Or just grab the English word from the line
    const match = line.match(/en:\s*\{\s*name:\s*'([^']+)'\s*\}/);
    if (match) {
        const enName = match[1];
        // Now find the start and end of translations: { ... }
        // We strictly know the line looks like: { code: '...', translations: { en: { name: '...' }, hi: { name: '...' } }, ... }
        // Let's replace translations: { .*?\} \s*\} (non-greedy till the double brace closes)
        const newTranslations = `translations: ${getTranslations(enName)}`;
        // To be absolutely safe, let's use indexOf
        const startIdx = line.indexOf('translations: {');
        let endIdx = -1;
        // Count braces
        let braceCount = 0;
        ...
        if (startIdx !== -1 && endIdx !== -1) {
            lines[i] = line.substring(0, startIdx) + newTranslations + line.substring(endIdx + 1);
            replacedCount++;
        }
    }
}
...
console.log(`Replaced precisely translation blocks line-by-line using brace counting. Count: ${replacedCount}`);
