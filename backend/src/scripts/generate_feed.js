...
const buildTrendingGrid = async (lang) => {
    const trendingImagesDeep = await Image.find({ is_deleted: { $ne: true }, language: { $in: ['neutral', lang] } })
        .select(`s3_key share_text media_type downloads_count event_id is_standalone standalone_category has_overlay greeting_id greeting_config quote_id quote_config is_s3_uploaded translations.en translations.${lang}`)
        .sort({ downloads_count: -1 })
        .limit(50)
        .populate({
            path: 'event_id',
            select: `vibes translations.en translations.${lang}`,
            populate: { path: 'vibes', select: `code icon color translations.en translations.${lang}` }
        });
    ...
    const trendingItems = trendingImagesDeep.map(img => ({
        ...
        vibes: (img.event_id && img.event_id.vibes) ? img.event_id.vibes.map(v => v.code) : []
    }));
...
const buildUpcomingHorizontal = async (allEvents, lang, today) => {
    ...
    for (const evt of upcomingEvents) {
        const primaryImage = await Image.findOne({ event_id: evt._id, is_deleted: { $ne: true }, language: { $in: ['neutral', lang] } }).sort({ created_at: 1 });
        const galleryImages = await Image.find({ event_id: evt._id, is_deleted: { $ne: true }, language: { $in: ['neutral', lang] } })
            .sort({ created_at: 1 })
            .limit(5);
